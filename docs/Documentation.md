...
## 1. Overview
The **Logic Gate Simulator** is an interactive JavaFX application that allows users to design and simulate digital logic circuits. The program provides a Graphical User Interface (GUI) to place logic gates (such as AND, OR, NOT) via drag-and-drop, wire them together, and observe the circuit behavior in real-time.
...
### 4.1 Base Classes
* **`DraggableGate`:** This class enables object movement via mouse and updates the component's image based on its state (e.g., light on/off). It also supports a **context menu** (right-click) allowing the user to delete the component.
...
* **Protection:** To prevent infinite loops (e.g., in oscillating circuits), the Runner stops calculation after a safety limit of 1000 steps.
...
1.  **User Action:** The user presses a button in the UI (`ButtonComponent`).
2.  **Logic Update:** The button sets its logical value in the backend (`ButtonElement`) to `true` and informs the `Runner`.
3.  **Simulation:** The `Runner` instructs all connected gates to re-evaluate their inputs (`calculateNewOutput`).
4.  **Propagation:** The signal travels through the connections. If a state changes (e.g., LED turns on), a callback is sent to the UI.
5.  **Visualization:** The UI (`LedComponent`) swaps its image (`LedOn.png`) so the user sees the result.